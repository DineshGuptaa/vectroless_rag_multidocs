          ...
          <TabsContent value="api-keys" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {modelSettings.llm_provider === 'ollama'
                    ? 'Not required when using local Ollama LLM'
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <Input
                      ...
                      placeholder={modelSettings.llm_provider === 'ollama' ? 'No API key needed (Ollama)' : "sk-..."}
                      className="pr-10"
                      disabled={modelSettings.llm_provider === 'ollama'}
                    />
                    <Button
                      variant="ghost"
                      size="icon"
                  ...
                  <Button
                    onClick={verifyKey}
                    disabled={!apiKey || verifying || modelSettings.llm_provider === 'ollama'}
                  >
                    {modelSettings.llm_provider === 'ollama' ? (
                      'Not Needed'
                    ) : (verifying ? (
                ...
                <div className="rounded-lg bg-muted p-3 text-sm">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="space-y-1">
                      {modelSettings.llm_provider === 'ollama' ? (
                        <>
                          <p>Using Ollama with local Llama 3.2 3B model</p>
                          <p className="text-xs text-muted-foreground">No API key required. The model runs locally in Docker.</p>
                        </>
                      ) : (
                        <>
          ...
          <TabsContent value="model" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Configure LLM provider and model for query processing
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-2">
                    Choose between OpenAI cloud API or local Ollama inference
                  </p>
                  <Select
                    value={modelSettings.llm_provider || 'openai'}
                    onValueChange={(value) =>
                      setModelSettings({...modelSettings, llm_provider: value as 'openai' | 'ollama'})
                    }
                  >
                    <SelectTrigger className="w-full mt-1">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-2">
                    Model used for both tree search (Stage 1) and answer generation (Stage 2)
                  </p>
                  <Select
                  ...
                  >
                    ...
                    <SelectContent>
                      {modelSettings.llm_provider === 'ollama' ? (
                        <>
                          <SelectItem value="llama3.2:3b">Llama 3.2 3B (Recommended)</SelectItem>
