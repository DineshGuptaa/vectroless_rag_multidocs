...
async def generate_tree(request: TreeGenerationRequest):
    ...
    try:
        ...
        # Step 2: Get document file from storage
        ...
        logger.info(f"Downloaded document: {len(pdf_bytes)} bytes")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if settings_response.status_code == 200:
                ...
                # Use defaults
                tree_config = {
                    "model": "llama3.2:3b" if LLM_PROVIDER == "ollama" else "gpt-4o-2024-11-20",
        ...
        # Inject model from model config (not part of tree settings)
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                model_resp = await client.get(f"{SETTINGS_SERVICE_URL}/settings/model")
                if model_resp.status_code == 200:
                    model_config = model_resp.json()
                    tree_config["model"] = model_config.get("model", tree_config.get("model", "llama3.2:3b" if LLM_PROVIDER == "ollama" else "gpt-4o-2024-11-20"))
        except Exception as e:
            logger.warning(f"Failed to get model config: {e}")
            tree_config.setdefault("model", "llama3.2:3b" if LLM_PROVIDER == "ollama" else "gpt-4o-2024-11-20")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if LLM_PROVIDER != "ollama" and key_response.status_code != 200:
                raise HTTPException(status_code=500, detail="OpenAI API key not configured. Please add your API key in settings.")
            api_key = key_response.json().get("key", "") if key_response.status_code == 200 else ""
        ...
        result = await page_index_main(pdf_stream, tree_config, api_key, progress_callback=emit_progress, filename=doc_info["filename"])
