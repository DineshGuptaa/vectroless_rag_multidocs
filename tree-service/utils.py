...
logger.info(f"utils.py loaded: LLM_PROVIDER={LLM_PROVIDER}, OLLAMA_BASE_URL={OLLAMA_BASE_URL}")
...
async def ChatGPT_API_async(model: str, prompt: str, api_key: str, max_retries: int = 3, max_tokens: int = 512) -> str:
    """Call LLM API asynchronously with retries (OpenAI or Ollama)."""
    messages = [{"role": "user", "content": prompt}]

    if LLM_PROVIDER == "ollama":
        api_url = f"{OLLAMA_BASE_URL}/v1/chat/completions"
        headers = {"Content-Type": "application/json"}
        logger.info(f"Ollama API URL: {api_url}, model: {model}")
        payload = {
            "model": model,
            "messages": messages,
            "temperature": 0,
    ...
    for i in range(max_retries):
        try:
            ...
            logger.error(f"LLM API error (attempt {i+1}/{max_retries}): type={type(e).__name__}, msg='{e}'")
            if i < max_retries - 1:
...
def extract_json(content: str) -> Dict[str, Any]:
    ...
    if not content or content.strip() == '':
        logger.error("extract_json: empty content")
        return {}
    ...
    try:
        ...
        if start_idx != -1:
            ...
            # Try to find JSON by looking for { and matching }
        ...
        logger.debug(f"Raw response snippet: {content[:200]}")
        try:
...
def _read_stream_as_text(stream: BytesIO) -> str:
    """Read a BytesIO stream as text (for .txt/.md files)."""
...
def get_page_tokens(pdf_path: Any, model: str = "gpt-4o-2024-11-20", pdf_parser: str = "PyPDF2", filename: str = "") -> List[Tuple[str, int]]:
    """Extract text and token counts from each page of a document.

    Supports PDF (via pypdf or PyMuPDF), DOCX (via python-docx), TXT, and MD.
    ...
    if ext == '.docx':
        if isinstance(pdf_path, BytesIO):
            full_text = _extract_docx_text(pdf_path)
        else:
            with open(pdf_path, 'rb') as f:
                full_text = _extract_docx_text(BytesIO(f.read()))
        token_length = len(enc.encode(full_text, disallowed_special=()))
        return [(full_text, token_length)]

    elif ext in ('.txt', '.md'):
        if isinstance(pdf_path, BytesIO):
            full_text = _read_stream_as_text(pdf_path)
        else:
    ...
    if pdf_parser == "PyPDF2":
        ...
        for page_num in range(len(pdf_reader.pages)):
            ...
            token_length = len(enc.encode(page_text, disallowed_special=()))
            ...
            if isinstance(data[i], dict) and 'physical_index' in data[i]:
                if isinstance(data[i]['physical_index'], str):
                    ...
                    if val.startswith('<physical_index_'):
                        ...
                        try:
                            data[i]['physical_index'] = int(val)
                        except (ValueError, TypeError):
                            pass
    elif isinstance(data, str):
        val = data
        if val.startswith('<physical_index_'):
            data = int(val.split('_')[-1].rstrip('>').strip())
        elif val.startswith('physical_index_'):
            data = int(val.split('_')[-1].strip())
        else:
            try:
                data = int(val)
            except (ValueError, TypeError):
...
def get_number_of_pages(pdf_path: Any, filename: str = "") -> int:
    """Get total number of pages/sections in a document.

    For PDF: returns page count.
    For non-PDF (DOCX, TXT, MD): returns 1 (treated as single-page).
    """
    ext = os.path.splitext(filename)[1].lower() if filename else ''
